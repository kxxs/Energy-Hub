...
for hour = 1:24
    ...
    result(hour).V_In = double(V_In);
    result(hour).Cost = double(Cost);
    result(hour).V = double(V);
    result(hour).Demand = Demand(:,hour);
    result(hour).V_Out = double(V_Out);
    Elec_In(1,hour) = result(hour).V_In(1);
    Gas_In(1,hour) = result(hour).V_In(2);
end
